...
data_file = 'data/BuildingData_new.csv'
...
values = dataset.values   # (35098, 9)

# ensure all data is float
values = values.astype('float32')      # (43800, 8)

# Column Wise (9 variables)
# Avg_ZoneTemp
# OAT
# RAT
# OA_Damper_CMD
...
reframed = reframed.values     # (34787, 108)

n_obs = Tx * inp_var   # 90

X = reframed[:, : n_obs]    # (34787, 90)
y1 = reframed [:, n_obs :]  # (34787, 18)

y = np.zeros ((y1.shape[0], Ty))  # (34787, 2)

# Select only Avg_ZoneTemp
for i in range(0, Ty):
    index = inp_var * i    # 0, 9, 18, 27, 36
    y [:, i] = y1 [:, index]

# Input Data
X = X.reshape((X.shape[0], Tx, inp_var))  # (34787, 10, 9)
...
# Scale features
x_train, y_train = scale_features (x_train, y_train)     # (24350, 10, 9), (24350, 2)
x_val, y_val = scale_features (x_val, y_val)    # (6958, 10, 9), (6958, 2)
x_test, y_test = scale_features (x_test, y_test)     # (3479, 10, 9), (3479, 2)